...
PROCESSED_DIR = os.path.join(PROJECT_ROOT, "voice_dataset", "processed")
...
async def synthesize_speech_async(
    ...
    if not output_path:
        filename = f"tts_{abs(hash(text)) % 1000000}.mp3"
    ...
    try:
        communicate = edge_tts.Communicate(
            ...
            voice=voice,
            rate=rate,
            pitch=pitch if pitch.startswith(("+", "-")) else f"+{pitch}"
        )
